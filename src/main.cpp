...
void process_report(string speak);
void process_log();
...
void build(){
    ...
    fstream detailedf_log("detailedf_log"); if (!detailedf_log) cnt++;
    fstream operation_log("operation_log"); if (!operation_log) cnt++;
    if (cnt != 0){
        ofstream books("book");
        ofstream users("user");
        ...
        ofstream detailedf_logs("detailedf_log");
        ofstream operation_logs("operation_log");
        ...
        detailedf_logs.write(reinterpret_cast<char *>(&number), sizeof(number));
        operation_logs.write(reinterpret_cast<char *>(&number), sizeof(number));
...
void process_su(string speak){
    ...
    if (current_user.priority > a.priority){
        if (password.empty() || password == a.passwd){
            ...
            current_user = log_stack[log_status];
            string user_id = current_user.user_id;
            operLinklist.add_operation(user_id,speak);
            return;
        }
...
void process_import(string speak){
    ...
    book bk;
    search.seekg(offset);
    search.read(reinterpret_cast<char *>(&bk), sizeof(bk));
    search.close();
    string bk_isbn = bk.isbn;
    detail_msg y(bk_isbn,quanti,'-',total);
    detailLog.push_back(y);
...
void process_buy(string speak){
    ...
    search.close();
    string bk_isbn = bk.isbn;
    detail_msg BUY(bk_isbn,quanti,'+',money);
    detailLog.push_back(BUY);
...
void process_report(string speak){
    int i = 0;
    string token, other;
    for( ; i < speak.length() && !isspace(speak[i]) ; ++i);//skip 'report'
    for( ; i < speak.length() &&  isspace(speak[i]) ; ++i);//skip space
    for( ; i < speak.length() && !isspace(speak[i]) ; ++i)token += speak[i];
        if (token.empty()){
        cout<<"Invalid\n";return;
    }
    for( ; i < speak.length() &&  isspace(speak[i]) ; ++i);
    for( ; i < speak.length() && !isspace(speak[i]) ; ++i)other += speak[i];
        if (other.empty()){
        cout<<"Invalid\n";return;
    }
...
void process_log(){
    cout<<"Hello!"<<endl;
    cout<<"Our bookstore's financial report is :"<<endl;
    process_report("report finance");
    cout<<"Every employee's operations are as follows:"<<endl;
    process_report("report employee");
}
