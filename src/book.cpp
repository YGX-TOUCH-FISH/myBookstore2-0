...
detail_msg::detail_msg(string ISBN, int QUANTI, char EXIM, double BUDGET) {
    for(int i = 0 ; i < sizeof(isbn);  ++i){
        isbn[i] = ISBN[i];
    }
    quanti = QUANTI;
...
void detail_msg::show() const {
    cout<<isbn<<" "<<quanti<<" "<<exim<<" "<<fixed<<setprecision(2)<<budget;
}

detail_msg &detail_msg::operator=(const detail_msg &ex){
    if (this == &ex)return *this;
    for(int i = 0 ; i < sizeof(isbn) ; ++i)isbn[i] = ex.isbn[i];
    quanti = ex.quanti;
    exim = ex.exim;
    budget = ex.budget;
    return *this;
...
void detail_log::push_back(detail_msg ex) {
    reopen();
    detail_msg a(ex);
    scanner.seekp(0,ios::end);
    scanner.write(reinterpret_cast<char *>(&a), sizeof(a));
...
void detail_log::show() {
    if (getnumber() == 0){
        cout<<endl;return;
    }
    detail_msg r;
    int pointer = sizeof(int);
    while (pointer != getnumber()* sizeof(detail_msg) + sizeof(int)){
        scanner.seekg(pointer);
        scanner.read(reinterpret_cast<char *>(&r), sizeof(r));
        r.show();
        cout<<endl;
        pointer += sizeof(detail_msg);
    }
}
...
void detail_log::open() {
    scanner.open("detailedf_log");
}

void detail_log::reopen() {
    scanner.close();
    scanner.open("detailedf_log");
}
